...
export class MapRenderer {
  ...
  registerMap(map: mapboxgl.Map) {
    this.map = map;
    new tileLayer.TileLayer(map, this.genOnLoadTileCanvasHandler())
  }
  ...
  private genOnLoadTileCanvasHandler() {
    // TODO: potentially better way to handle this?
    const renderer = this;
     
    return function (tile: tileLayer.Tile) {
      console.log(tile);
      ...
      if (tile.z === 9) {
        const x = tile.x;
        const y = tile.y;
        const fowTile = renderer.fogMap.tiles[fogMap.Map.makeKeyXY(x, y)];
        // fowTile?.
